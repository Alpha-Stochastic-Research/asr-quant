...
class QuantLab:
    ...
    def discover_hypotheses(
        ...
        **kwargs: Any,
        ...
        """Discover falsifiable data-driven hypotheses from the current lab.

        By default the scan uses returns rather than price levels. Pass
        ``data=...`` for custom predictors/targets or ``use_returns=False`` when
        level-series structure is the object of study.
        """
        from .hypotheses import from_data

        source = pd.DataFrame(data) if data is not None else (self.returns if use_returns else self.prices)
        return from_data(source, domain=domain, **kwargs)
    ...
    def factor_analysis(
        ...
        *,
        ...
        """Run PCA factor analysis on lab returns or prices."""
        from .factors import pca

        source = self.returns if use_returns else self.prices
        return pca(source, n_components=n_components, standardize=standardize)
    ...
    def portfolio_risk(
        ...
        *,
        ...
        """Build a portfolio-risk report using the lab return history.

        If a weight path is supplied, the latest available row is used as the
        fixed risk snapshot. When omitted, the most recently generated strategy
        weights are used.
        """
        from .risk import portfolio_risk_report

        active = weights if weights is not None else self.last_weights
        if active is None:
            raise RuntimeError("provide weights or generate a strategy before requesting portfolio risk")
        if isinstance(active, pd.DataFrame):
            usable = active.dropna(how="all")
            if usable.empty:
                raise ValueError("weights contain no usable row")
            snapshot = usable.iloc[-1]
        else:
            snapshot = pd.Series(active, dtype=float)
        return portfolio_risk_report(
            self.returns, snapshot, level=level, annualization=annualization
        )
    ...
    def alpha_analysis(
        ...
        *,
        ...
        """Evaluate a cross-sectional signal against forward returns."""
        from .alpha import analyze_signal, forward_returns

        future = forward_returns(self.prices, periods=horizon)[horizon]
        return analyze_signal(signal, future, quantiles=quantiles, min_assets=min_assets)
