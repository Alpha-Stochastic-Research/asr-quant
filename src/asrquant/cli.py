...
from .data import load_prices, load as load_data, validate as validate_data
...
def _parser() -> argparse.ArgumentParser:
    ...
    sub.add_parser("info", help="show the installed ASRQuant release and public research namespaces")

    doctor = sub.add_parser("doctor", help="check the local ASRQuant research environment")
    doctor.add_argument("--json", action="store_true", dest="as_json")

    validate_cmd = sub.add_parser("validate", help="validate a local research dataset without mutating it")
    validate_cmd.add_argument("path")
    validate_cmd.add_argument("--date-column", default=None)
...
def _doctor_payload() -> dict[str, object]:
    required = ["numpy", "pandas", "scipy", "matplotlib", "plotly", "statsmodels", "scikit-learn"]
    optional = ["yfinance", "pyarrow", "cvxpy", "arch", "shap", "hmmlearn"]
    ...
    required_versions = {name: version_of(name) for name in required}
    optional_versions = {name: version_of(name) for name in optional}
    with tempfile.TemporaryDirectory(prefix="asrquant-doctor-") as tmp:
        probe = Path(tmp) / "write-test"
        probe.write_text("ok", encoding="utf-8")
        writable = probe.read_text(encoding="utf-8") == "ok"

    return {
        "asrquant": __version__,
        "python": platform.python_version(),
        "platform": platform.platform(),
        "required_dependencies": required_versions,
        "optional_dependencies": optional_versions,
        "temporary_storage_writable": writable,
        "status": "PASS" if all(required_versions.values()) and writable else "FAIL",
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "info":
        payload = {
            ...
            "python": platform.python_version(),
            "namespaces": [
                "data", "hypotheses", "alpha", "factors", "risk", "microstructure",
                "validation", "scenarios", "credit", "portfolio", "backtesting",
                "rates", "options", "calibration", "sensitivities", "research",
            ],
        }
    ...
    if args.command == "doctor":
        ...
        if args.as_json:
            ...
            print(f"ASRQuant       {payload['asrquant']}")
            print(f"Python         {payload['python']}")
            print(f"Environment    {payload['status']}")
            for name, version in payload["required_dependencies"].items():
                print(f"{name:<14} {version or 'MISSING'}")
            print("Optional extras:")
            for name, version in payload["optional_dependencies"].items():
                print(f"  {name:<12} {version or 'not installed'}")
        return 0 if payload["status"] == "PASS" else 2

    if args.command == "validate":
        frame = load_data(args.path, date_column=args.date_column)
