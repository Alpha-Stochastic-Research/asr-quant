...
class ValidationError(RuntimeError, ASRQuantError):
    """A research-validation procedure failed or could not be evaluated."""


class DataLeakageError(ValidationError):
    """Point-in-time or feature chronology violates the research contract."""


class SchemaError(DataValidationError):
    """A dataset does not satisfy the declared schema contract."""


class StaleDataError(DataValidationError):
    """A cached or provider dataset is older than its allowed freshness."""


class ScenarioError(RuntimeError, ASRQuantError):
    """A stress/scenario request is inconsistent or cannot be repriced."""


class ResearchError(RuntimeError, ASRQuantError):
    """A research experiment or artifact lineage operation failed."""
