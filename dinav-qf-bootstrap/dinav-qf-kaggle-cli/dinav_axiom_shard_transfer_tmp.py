...
def target_key(key: str) -> str | None:
    ...
    if not key.startswith('blocks.'): return None
    pieces = key.split('.')
    layer = pieces[1]
    tail = '.'.join(pieces[2:])
    suffix = {
...
def headers(content_type: str = 'application/octet-stream'):
    return {
        'apikey': SUPABASE_KEY,
...
def upload(path: Path, remote_name: str, content_type: str = 'application/octet-stream'):
    ...
    if len(payload) > 47_185_920:
        raise RuntimeError(f'transport object too large: {remote_name} {len(payload)}')
    last = None
    for attempt in range(1, 5):
        try:
            response = requests.post(url, headers=request_headers, data=payload, timeout=180)
...
config = LlamaConfig(
    vocab_size=int(raw['vocab_size']), hidden_size=int(raw['dim']), intermediate_size=int(raw['ffn_dim']),
    num_hidden_layers=int(raw['n_layers']), num_attention_heads=int(raw['n_heads']), num_key_value_heads=int(raw['n_kv_heads']),
    max_position_embeddings=int(raw['max_seq_len']), rope_theta=float(raw['rope_theta']), hidden_act='silu', rms_norm_eps=1e-6,
    attention_bias=False, mlp_bias=False, tie_word_embeddings=True, bos_token_id=int(special_ids['<bos>']),
    eos_token_id=int(special_ids['<eos>']), pad_token_id=int(special_ids['<pad>']), use_cache=True,
)
...
for source_name, tensor in state.items():
    ...
    if tuple(tensor.shape) != tuple(target[destination].shape):
        report(stage='MAPPING_SHAPE_FAILED', source_tensor=source_name, destination=destination)
        raise SystemExit(8)
...
tokenizer = PreTrainedTokenizerFast(tokenizer_file=str(tokenizer_path), bos_token='<bos>', eos_token='<eos>', unk_token='<unk>', pad_token='<pad>')
...
report(stage='SAFETENSORS_SAVED', logical_file_count=len(list(OUT.iterdir())))

reloaded = AutoModelForCausalLM.from_pretrained(OUT, local_files_only=True, trust_remote_code=False, torch_dtype=torch.float32)
...
for path in sorted(OUT.iterdir()):
    if path.is_file():
        logical_files.append({'name': path.name, 'size': path.stat().st_size, 'sha256': sha256_file(path)})

manifest = {
    'model_name': 'DINAV-Axiom-125M', 'format': 'huggingface-llama-compatible-safetensors',
    'source_checkpoint': checkpoint_path.name, 'source_checkpoint_sha256': source_sha,
    'source_tokenizer': tokenizer_path.name, 'source_tokenizer_sha256': sha256_file(tokenizer_path),
    'checkpoint_step': step, 'tokens_seen': int(checkpoint.get('tokens_seen', 0)), 'parameter_count': EXPECTED_PARAMS,
    'vocab_size': int(raw['vocab_size']), 'training_max_seq_len': int(raw['max_seq_len']),
    ...
    'production_loader_verified': True, 'generation_smoke_verified': True,
    'smoke_text': smoke_text[:200], 'logical_files': logical_files,
}
manifest_path = OUT / 'dinav_axiom_manifest.json'
manifest_path.write_text(json.dumps(manifest, indent=2), encoding='utf-8')
logical_files.append({'name': manifest_path.name, 'size': manifest_path.stat().st_size, 'sha256': sha256_file(manifest_path)})
report(stage='PRODUCTION_PACKAGE_VERIFIED', logical_files=len(logical_files))

transport_entries = []
for logical in logical_files:
    path = OUT / logical['name']
    if path.stat().st_size <= TRANSPORT_CHUNK:
        transport_entries.append({
            'logical_name': path.name, 'logical_size': path.stat().st_size, 'logical_sha256': sha256_file(path),
            'mode': 'direct', 'objects': [{'name': 'files/' + path.name, 'size': path.stat().st_size, 'sha256': sha256_file(path)}],
        })
        continue
    objects = []
    with path.open('rb') as src:
        index = 0
        while True:
            data = src.read(TRANSPORT_CHUNK)
            if not data: break
            chunk_name = f'chunks/{path.name}.part-{index:04d}'
            chunk_path = CHUNKS / chunk_name.replace('/', '__')
            chunk_path.write_bytes(data)
            objects.append({'name': chunk_name, 'size': len(data), 'sha256': hashlib.sha256(data).hexdigest(), 'local': str(chunk_path)})
            index += 1
    transport_entries.append({
        'logical_name': path.name, 'logical_size': path.stat().st_size, 'logical_sha256': sha256_file(path),
        'mode': 'chunked', 'objects': [{k: v for k, v in obj.items() if k != 'local'} for obj in objects],
    })

transport_manifest = {
    'transport_format': 'DINAV-Axiom Supabase 40MiB chunks v1',
    'production_loader_verified': True, 'generation_smoke_verified': True,
    'parameter_count': EXPECTED_PARAMS, 'source_checkpoint_sha256': source_sha,
    'logical_files': transport_entries,
}
transport_manifest_path = WORK / 'axiom-production-transport-manifest.json'
transport_manifest_path.write_text(json.dumps(transport_manifest, indent=2), encoding='utf-8')
report(stage='TRANSPORT_PACKAGE_VERIFIED', logical_files=len(transport_entries), transport_manifest_sha256=sha256_file(transport_manifest_path))
...
for entry in transport_entries:
    logical_path = OUT / entry['logical_name']
    if entry['mode'] == 'direct':
        upload(logical_path, 'files/' + logical_path.name)
        object_count += 1
    else:
        for obj in entry['objects']:
            local = CHUNKS / obj['name'].replace('/', '__')
            upload(local, obj['name'])
            object_count += 1
    report(stage='UPLOADING_PRODUCTION', completed_logical=entry['logical_name'], transport_objects=object_count)

upload(transport_manifest_path, 'axiom-production-transport-manifest.json', 'application/json')
object_count += 1
report(
    stage='UPLOAD_COMPLETE', parameter_count=EXPECTED_PARAMS, logical_files=len(transport_entries), transport_objects=object_count,
    source_checkpoint_sha256=source_sha, transport_manifest_sha256=sha256_file(transport_manifest_path),
)
